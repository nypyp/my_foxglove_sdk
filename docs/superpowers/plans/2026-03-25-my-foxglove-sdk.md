...
**Reference source (read-only):** `third-party/foxglove-sdk/` — must be cloned before any implementation task begins.
...
- `RawChannel` class:
  ...
  - Public static: `FoxgloveResult<RawChannel> create(string topic, string encoding, Schema schema, MessageCallback callback)`
...
- `WebSocketServer` class (PIMPL):
  ...
  - `void add_channel(RawChannel&)` — advertises, provides MessageCallback
  - `void remove_channel(uint32_t channel_id)` — unadvertises
...
- [ ] **Step 7: Create ch06_context example + Write tutorial doc**
...
- [ ] **Step 4: Build, run tests, verify with `mcap info` if available**
...
Test that verifies: start demo → connect scripted client → receives valid messageData frames → stop → verify `output.mcap` with `mcap info`.

- [ ] **Step 3: Write Chapter 9 tutorial document**

Cover architecture review, thread model, performance discussion, design retrospective, suggested exercises.
